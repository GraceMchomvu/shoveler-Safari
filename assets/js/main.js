(function ($) {
  ...
  if (document.querySelector("#navbars")) {
    ...
    const setStickyNav = (show) => {
      if (show === navShown) return;
      navShown = show;
      document.body.classList.toggle("is-nav-sticky", show);
      gsap.to("#navbars", {
        opacity: show ? 1 : 0,
        ...
        overwrite: "auto",
        onStart: () => {
          if (show) document.querySelector("#navbars")?.style.setProperty("pointer-events", "auto");
        },
        onComplete: () => {
          if (!show) document.querySelector("#navbars")?.style.setProperty("pointer-events", "none");
        },
      });
    };

    const hero = document.querySelector(".shoveler-hero");
    const pageHeader = document.querySelector(".vs-header");
    const trigger = hero || pageHeader || "body";
    const start = hero
      ? "bottom top"
      : pageHeader
        ? "bottom top"
